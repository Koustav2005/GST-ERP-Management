...
async function cleanupSVCEData() {
    ...
    try {
        ...
        if (await tableExists(client, 'sales_orders')) {
            console.log('2. Deleting sales orders...');
            await client.query('DELETE FROM sales_orders WHERE inventory_id IN (SELECT id FROM sales_inventory WHERE project_id IN (SELECT id FROM projects WHERE company_id = $1))', [svceCompanyId]);
        }

        if (await tableExists(client, 'sales_inventory')) {
            console.log('3. Deleting sales inventory...');
            await client.query('DELETE FROM sales_inventory WHERE project_id IN (SELECT id FROM projects WHERE company_id = $1)', [svceCompanyId]);
        }

        if (await tableExists(client, 'allocation_tasks')) {
            console.log('4. Deleting allocation tasks...');
        ...
        if (await tableExists(client, 'store_request_items')) {
            console.log('5. Deleting store request items...');
        ...
        if (await tableExists(client, 'bid_items') && await tableExists(client, 'vendor_bids') && await tableExists(client, 'vendor_demands')) {
            console.log('10. Deleting bid items...');
        ...
        if (await tableExists(client, 'vendor_bids') && await tableExists(client, 'vendor_demands')) {
            console.log('11. Deleting vendor bids...');
        ...
        if (await tableExists(client, 'demand_items')) {
            console.log('12. Deleting demand items...');
        ...
        if (await tableExists(client, 'vendor_demands')) {
            console.log('13. Deleting vendor demands...');
        ...
        if (await tableExists(client, 'requirement_items')) {
            console.log('14. Deleting requirement items...');
        ...
        if (await tableExists(client, 'requirements')) {
            console.log('15. Deleting requirements...');
        ...
        if (await tableExists(client, 'project_revisions')) {
            console.log('16. Deleting project revisions...');
        ...
        if (await tableExists(client, 'bill_of_materials')) {
            console.log('17. Deleting bill of materials...');
